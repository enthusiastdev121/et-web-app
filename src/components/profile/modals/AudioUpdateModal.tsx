...
import { formatAudioNodeOwn, formatVideoNodeOwn } from "network/apiFormatter/profile";
import { updateAudioRoute, updateVideoRoute, uploadVideoRoute } from "network/apis/ownProfile";
import React, { useState } from "react";
import { IoClose } from "react-icons/io5";
import styled from "styled-components";
import { AudioItemOwnD, VideoItemD, VideoItemOwnD } from "types/profile";

export default function AudioUpdateModal(props: Props) {
  const { open, onClose, id, onUpdate, uri } = props;
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setUpdating(true);
      const res = await axios.post(updateAudioRoute(id), raw, {
        headers: {
      ...
      setUpdating(false);
      if (res.data) {
        onUpdate(formatAudioNodeOwn(res.data));
      }
    } catch (err) {
      console.log("Err", err);
      setUpdating(false);
        ...
        <div className="px-4 pb-3">
          ...
          <div className="">
            ...
            <div className="flex flex-col">
              ...
              <div className="">
                <div className="text-sm font-semibold mb-1 w-full">Audio</div>
